...
#---------------------------------------
def get_max(proj_data, kpt_weight, atom_idx=0, orbital='d', alpha=0, beta=0, gamma=0, half_range=10, dtheta=1):
    ...
    for z_angle in np.linspace(z1_1,z1_2,2*int(half_range/dtheta)+1):
        for y_angle in np.linspace(z2_1,z2_2,2*int(half_range_y/dtheta)+1):
            for x_angle in np.linspace(z3_1,z3_2,2*int(half_range/dtheta)+1):
                ...
                #---------------------------------------
                rot_mat = R.from_euler('ZYX', [z_angle, y_angle, x_angle], degrees=False).as_matrix()
                ...
                #---------------------------------------
                # Choose a suitable metric
                #---------------------------------------
                #  #  weight_rotate is in [ikpt, iband, iorbital]
                #  #  find standard deviation of the weight for each kpt and orbital
                #  std_band = -np.std(weight_rotated[:,:,:].real,axis=1)
                #  #  dot the kpt weight to the list of std
                #  std = kpt_weight[:,np.newaxis]*std_band
                #  max_val = np.sum(std)

                #  # find max proj value of each orbital across all bands and kpts.
                #  max_val_band = np.max(weight_rotated[:,:,:].real,axis=1)
                #  #  max_val_kpt = kpt_weight[:,np.newaxis]*max_val_band
                #  max_val_kpt = max_val_band
                #  orbital_weight = np.array([0,0,1,0,0])
                #  #  orbital_weight = np.array([5./2.,5./2.,5./1.,5./2.,5./2])
                #  max_val_orb = orbital_weight[np.newaxis,:]*max_val_kpt
                #  #  max_val_kpt =  np.max(kpt_weight[:,np.newaxis]*max_val_band,axis=0)
                #  # and sum up, use this as metric
                #  max_val = np.sum(max_val_kpt)

                #  # find max proj value of each orbital across all bands and kpts.
                #  max_val_band = np.sum(weight_rotated[:,:,:].real,axis=1)
                #  max_val_kpt = kpt_weight[:,np.newaxis]*max_val_band
                #  orbital_weight = np.array([1,1,1,1,1])
                #  #  orbital_weight = np.array([5./2.,5./2.,5./1.,5./2.,5./2])
                #  max_val_orb = orbital_weight[np.newaxis,:]*max_val_kpt
                #  #  max_val_kpt =  np.max(kpt_weight[:,np.newaxis]*max_val_band,axis=0)
                #  # and sum up, use this as metric
                #  max_val = np.sum(max_val_kpt[0,:])

                #  # find max proj value of each orbital across all bands and kpts.
                #  max_val = np.max(weight_rotated[:,:,:].real,axis=1)
                #  max_val = kpt_weight[:,np.newaxis]*max_val
                #  orbital_weight = np.array([1,1,1,1,1])
                #  max_val = orbital_weight[np.newaxis,:]*max_val
                #  # and sum up, use this as metric
                ...
                max_val = np.max(weight_rotated[:,:,:].real,axis=1)
                number_of_significant_bands = np.sum(weight_rotated[:,:,:].real>0.8*max_val[:,np.newaxis,:],axis=1)
                #  print(number_of_significant_bands)
                max_val = max_val/number_of_significant_bands
                max_val = kpt_weight[:,np.newaxis]*max_val
                orbital_weight = np.array([1,1,1,1,1])
                max_val = orbital_weight[np.newaxis,:]*max_val 
                max_val = np.sum(max_val)
...
for iter in range(4):
    ...
    result = get_max(proj_data, kpt_weight, atom_idx, orbital, alpha,beta,gamma, half_range[iter], dtheta[iter])
    alpha = result[0]
    beta = result[1]
    gamma = result[2]
    #  print(result[3])
...
# print out matrix
from scipy.spatial.transform import Rotation as R
r = R.from_euler('ZYX', [result[0], result[1], result[2]], degrees=True)
print(r.as_matrix())
